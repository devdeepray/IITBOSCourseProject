...
int Init_Inode_Manager() {
	...
	if (inode_manager.max_bitmap_size < inode_manager.bitmap_number_of_blocks*BLOCK_SIZE*sizeof(char)) {
		inode_manager.max_bitmap_size = inode_manager.bitmap_number_of_blocks*BLOCK_SIZE*sizeof(char);
	}
			...
			if(buf[j] != (char)0xFF) // If found a 0 bit
			{
				...
				for(k = 0; k < 8; ++k)
				{
					...
					if((buf[j] | (char)0x1 << (7 - k))  != buf[j])
					{
						...
						if(freeInodeNum > disk_superblock.numInodeBlocks * (BLOCK_SIZE / sizeof(Inode)))
						{
							rc = -1;
						}
...
int Free_Inode(int inodeNo) {
	...
	if(inodeNo > disk_superblock.numInodeBlocks * (BLOCK_SIZE / sizeof(Inode))) return -1;
	// Calculate the block number for the corresponding bit
	int inodeBlockNo = disk_superblock.firstInodeBitmapBlock + (inodeNo / (disk_superblock.blockSizeInBytes * sizeof(char)));
	// Calculate char number for corresponding bit
	int charNumber = (inodeNo % disk_superblock.blockSizeInBytes) / sizeof(char);
...
int Create_New_Inode(InodeMetaData meta_data, int* newInodeNo) {
	...
	int blockNo = (*newInodeNo) / (BLOCK_SIZE / sizeof(Inode)) + disk_superblock.firstInodeBlock;
	
	int indexInBlock = (*newInodeNo) % (BLOCK_SIZE / sizeof(Inode));
	Inode *buf;
	rc = rc | Get_Into_Cache(blockNo, (char**)&buf);
	
	memcpy((void*)&buf[indexInBlock], (void*) &newInode, sizeof(Inode));
...
int Read_Inode_From_Disk(Inode *inode, int inodeNo) {
	...
	if (!rc) {
		int inodeStart = (sizeof(Inode) * (inodeNo % inodesPerBlock)) / sizeof(char);
		memcpy((void*) inode, (void*) &block[inodeStart], sizeof(Inode));
	}
...
int Write_Inode_To_Disk(Inode *inode, int inodeNo) {
	...
	if (!rc) {
		int inodeStart = (sizeof(Inode) * (inodeNo % inodesPerBlock)) / sizeof(char);
		memcpy((void*) &block[inodeStart], (void*) inode, sizeof(Inode));
	}
	Set_Dirty(blockNo);
	Unfix_From_Cache(blockNo);
	return rc;
...
int Allocate_Upto(int inodenum, int allocate_size)
{
	...
	for(; current_num_blocks < final_num_blocks && current_num_blocks < BLOCK_SIZE / sizeof(int); current_num_blocks++)
	{
	...
	{
		if(orig_num_blocks <= INODE_BASE_SIZE + (i + 1) * BLOCK_SIZE / sizeof(int))
		{
...
int Get_Block_For_Byte_Address(Inode* inode, int byte_address, int* blocknum)
{
	...
	if(block_num < BLOCK_SIZE / sizeof(int))
	{
		int *buf, rc;
	...
	int first_level_index = block_num / (BLOCK_SIZE / sizeof(int));
	int second_level_index = block_num % (BLOCK_SIZE / sizeof(int));
	int *buf;
